...
fn walk(
    ...
    let mut pretty: Vec<(usize, usize)> = Vec::new();
    pretty.push((cx as usize, cy as usize));
    for i in 1..4 {
        ...
        for x in 0..lines[y].len() {
            ...
            for (dx, dy) in directions.into_iter() {
                if !check_x(x, y, &lines, dx, dy) {
                    ...
                    // funny debug stuff
                    pretty.push((x as usize, y as usize));
                    let tempa = x.checked_add_signed(dx as isize).unwrap();
                    let tempb = y.checked_add_signed(dy as isize).unwrap(); 
                    pretty.push((tempa, tempb));
                    let tempa = tempa.checked_add_signed((-2 * dx) as isize).unwrap();
                    let tempb = tempb.checked_add_signed((-2 * dy) as isize).unwrap(); 
                    pretty.push((tempa, tempb));
                }
            }
            if cross {
    ...
    for y in 0..lines.len() {
        for x in 0..lines[y].len() {
            if pretty.contains(&(x, y)) {
                ...
                print!("{}", lines[y][x].to_string().green());
            }
        }
        println!()
    }
